  ...
  try {
    ...
    Object.keys(historyByInvestment).forEach(invId => {
      ...
      entries.forEach(entry => {
        console.log(`     - ${entry.date.toISOString().split('T')[0]}: ${entry.operation} | totalValue: ${entry.totalValue?.toFixed(2) || 'N/A'}€ | operationAmount: ${entry.operationAmount?.toFixed(2) || 'N/A'}€`);
      });
    });
    ...
    allVariations.forEach(v => {
      ...
      console.log(`   ${isCurrent ? '✅' : '❌'} ${v.date.toISOString().split('T')[0]}: ${v.totalValue.toFixed(2)}€ (Investment: ${v.investment})`);
    });
    ...
    allInvestmentsWithSimilarName.forEach(inv => {
      ...
      console.log(`   ${isCurrent ? '✅ ACTUAL' : '❌ ELIMINADA'} - ${inv.name} (${inv._id}): ${value.toFixed(2)}€`);
    });
    ...
    if (variationDec31) {
      console.log(`   DailyVariation: ${variationDec31.totalValue.toFixed(2)}€`);
      console.log(`   Diferencia con valor actual: ${(currentValue - variationDec31.totalValue).toFixed(2)}€`);
    } else {
      ...
      if (historyDec31) {
        console.log(`   InvestmentHistory: ${historyDec31.totalValue.toFixed(2)}€`);
        console.log(`   Diferencia con valor actual: ${(currentValue - historyDec31.totalValue).toFixed(2)}€`);
      } else {
