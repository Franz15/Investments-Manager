...
async function justifyAnnualReturn() {
  try {
    ...
    currentValues.forEach((inv) => {
      console.log(
        `  ${inv.name.padEnd(50)} ${inv.value.toFixed(2).padStart(12)}€`,
      );
    });
    console.log("-".repeat(80));
    ...
    if (variationsDec31.length > 0) {
      ...
      variationsDec31.forEach((v) => {
        const inv = investments.find(
          (i) => i._id.toString() === v.investment.toString(),
        );
        ...
        console.log(
          `  ${(inv?.name || "N/A").padEnd(50)} ${value.toFixed(2).padStart(12)}€ (DailyVariation 31/12)`,
        );
      });
    }
    ...
    for (const inv of investments) {
      ...
      if (
        ...
        if (lastVariation && lastVariation.totalValue) {
          ...
          console.log(
            `  ${inv.name.padEnd(50)} ${lastVariation.totalValue.toFixed(2).padStart(12)}€ (DailyVariation ${lastVariation.date.toISOString().split("T")[0]})`,
          );
        } else {
          ...
          if (invCapitalBeforeYear > 0) {
            ...
            console.log(
              `  ${inv.name.padEnd(50)} ${invCapitalBeforeYear.toFixed(2).padStart(12)}€ (Capital invertido hasta 31/12)`,
            );
          }
        }
      }
    ...
    console.log("-".repeat(80));
    console.log(
      `  VALOR BASE (yearStartValue): ${yearStartValue.toFixed(2).padStart(12)}€`,
    );
    console.log(
      `  VALOR AÑADIDO (sin registro): ${missingInvestments
        .reduce((sum, inv) => sum + inv.value, 0)
        .toFixed(2)
        .padStart(12)}€`,
    );
    console.log(
      `  VALOR REAL AL INICIO DEL AÑO: ${actualYearStartValue.toFixed(2).padStart(12)}€\n`,
    );
      ...
      if (entryDate >= yearStart) {
        if (entry.operation === "creation") {
          ...
          if (amount >= 0) {
            ...
            console.log(
              `  ${entry.date.toISOString().split("T")[0]} - ${entry.operation.padEnd(10)} - ${(entry.investment?.name || "N/A").padEnd(40)} +${amount.toFixed(2).padStart(10)}€ (NUEVA)`,
            );
          }
          ...
          if (amount >= 0) {
            ...
            console.log(
              `  ${entry.date.toISOString().split("T")[0]} - ${entry.operation.padEnd(10)} - ${(entry.investment?.name || "N/A").padEnd(40)} +${amount.toFixed(2).padStart(10)}€ (EXISTENTE)`,
            );
          }
        } else if (
          ...
          console.log(
            `  ${entry.date.toISOString().split("T")[0]} - ${entry.operation.padEnd(10)} - ${(entry.investment?.name || "N/A").padEnd(40)} -${amount.toFixed(2).padStart(10)}€`,
          );
        }
      }
    }
    ...
    console.log("-".repeat(80));
    console.log(
      `  Aportaciones a inversiones NUEVAS: ${capitalAddedToNewInvestments.toFixed(2).padStart(12)}€`,
    );
    console.log(
      `  Aportaciones a inversiones EXISTENTES: ${capitalAddedToOldInvestments.toFixed(2).padStart(12)}€`,
    );
    console.log(
      `  Retiros: ${capitalWithdrawnThisYear.toFixed(2).padStart(12)}€\n`,
    );

    // Calcular rendimiento anual
    const annualReturn =
      currentValue -
      actualYearStartValue -
      capitalAddedToOldInvestments +
    ...
    console.log("=".repeat(80));
    console.log(
      `Fórmula: annualReturn = currentValue - actualYearStartValue - capitalAddedToOldInvestments + capitalWithdrawnThisYear\n`,
    );
    console.log(
      `  Valor actual:                    ${currentValue.toFixed(2).padStart(15)}€`,
    );
    console.log(
      `  Valor al inicio del año:          ${actualYearStartValue.toFixed(2).padStart(15)}€`,
    );
    console.log(
      `  Aportaciones a existentes:        ${capitalAddedToOldInvestments.toFixed(2).padStart(15)}€`,
    );
    console.log(
      `  Retiros:                          ${capitalWithdrawnThisYear.toFixed(2).padStart(15)}€`,
    );
    console.log(
      `  ─────────────────────────────────────────────────────────────`,
    );
    console.log(
      `  RENDIMIENTO ANUAL:                 ${annualReturn.toFixed(2).padStart(15)}€`,
    );
    console.log(
      `  RENDIMIENTO ANUAL (%):              ${annualReturnPercent.toFixed(2).padStart(15)}%\n`,
    );
    ...
    console.log(
      "Inversión".padEnd(50) +
        "Valor 1/1".padStart(15) +
        "Valor Actual".padStart(15) +
        "Cambio".padStart(15),
    );
    ...
    investments.forEach((inv) => {
      ...
      const contributionsToThisInv = contributions2026.filter((c) => {
        const entry = allHistoryEntries.find(
          (e) =>
            e.investment?.toString() === invId &&
            new Date(e.date).toISOString().split("T")[0] === c.date,
        );
        return (
          entry && (entry.operation === "add" || entry.operation === "creation")
        );
      });
      ...
      if (
        invYearStartValue > 0 ||
        invCurrentValue > 0 ||
        contributionsToThisInv.length > 0
      ) {
    ...
    console.log("-".repeat(95));
    console.log(
      `${"TOTAL".padEnd(50)}${actualYearStartValue.toFixed(2).padStart(15)}€${currentValue.toFixed(2).padStart(15)}€${annualReturn.toFixed(2).padStart(15)}€\n`,
    );
