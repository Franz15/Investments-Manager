...
async function checkInvestmentsDetail() {
  try {
    ...
    for (const userId of users) {
      ...
      }).populate('account').populate('subAccount').sort({ createdAt: 1 });
      ...
      for (const inv of investments) {
        ...
        console.log(`Tipo: ${inv.type}${inv.isAutomatedPortfolio ? ' (Cartera Automatizada)' : ''}`);
        console.log(`Símbolo: ${inv.symbol || 'N/A'}`);
        console.log(`Cuenta: ${inv.account?.name || inv.account || 'N/A'}`);
        console.log(`Subcuenta: ${inv.subAccount?.name || inv.subAccount || 'N/A'}`);
        ...
        if (inv.isAutomatedPortfolio) {
          ...
          console.log(`Precio de compra: ${inv.purchasePrice?.toFixed(2) || 0}€`);
          console.log(`Precio medio: ${inv.averagePurchasePrice?.toFixed(2) || inv.purchasePrice?.toFixed(2) || 0}€`);
        ...
        if (capitalOperations.length > 0) {
          capitalOperations.forEach((op, idx) => {
            console.log(`  ${idx + 1}. ${op.date.toISOString().split('T')[0]} - ${op.operation.toUpperCase()}: ${op.operationAmount.toFixed(2)}€`);
        ...
        if (firstVariation || lastVariation) {
          console.log(`\nVariaciones diarias:`);
          if (firstVariation) {
            console.log(`  Primera: ${firstVariation.date.toISOString().split('T')[0]} - Valor: ${firstVariation.totalValue?.toFixed(2) || 0}€`);
          }
          if (lastVariation) {
            console.log(`  Última: ${lastVariation.date.toISOString().split('T')[0]} - Valor: ${lastVariation.totalValue?.toFixed(2) || 0}€`);
          }
        ...
        if (historyWithValue.length > 0) {
          ...
          historyWithValue.forEach((h, idx) => {
            console.log(`  ${idx + 1}. ${h.date.toISOString().split('T')[0]} - Valor: ${h.totalValue.toFixed(2)}€${h.operation ? ` (${h.operation})` : ''}`);
          });
        }
      }
      ...
      console.log(`Neto: ${(totalContributions - totalWithdrawals).toFixed(2)}€`);
      ...
      subAccounts.forEach(sa => {
        console.log(`  - ${sa.name} (${sa.type}): ${sa.balance.toFixed(2)}€${sa.initialDate ? ` - Fecha inicial: ${new Date(sa.initialDate).toISOString().split('T')[0]}` : ' - Sin fecha inicial'}`);
      });
