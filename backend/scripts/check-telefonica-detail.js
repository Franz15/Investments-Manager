...
async function checkTelefonicaDetail() {
  try {
    ...
    console.log(
      `   Precio compra: ${telefonica.purchasePrice || telefonica.averagePurchasePrice || "N/A"}€`,
    );
    console.log(
      `   Valor actual: ${telefonica.isAutomatedPortfolio ? telefonica.currentPrice : (telefonica.quantity || 0) * (telefonica.currentPrice || 0)}€`,
    );
    console.log("");
    ...
    for (const entry of history) {
      ...
      const price = (entry.currentPrice || entry.operationPrice || 0)
        .toFixed(2)
        .padStart(10);
      const totalValue = (entry.totalValue || 0).toFixed(2).padStart(13);
      const operationAmount = (
        entry.operationAmount !== null && entry.operationAmount !== undefined
      ...
      if (entry.operation === "creation" || entry.operation === "add") {
        ...
        if (amount >= 0) {
          ...
          console.log(
            `  → Capital añadido: ${amount.toFixed(2)}€ (total acumulado: ${totalInvested.toFixed(2)}€)`,
          );
        }
        ...
        totalInvested -= amount;
        console.log(
          `  → Capital retirado: ${amount.toFixed(2)}€ (total acumulado: ${totalInvested.toFixed(2)}€)`,
        );
      }
    }
    ...
    console.log(
      `📈 Rendimiento: ${(currentValue - totalInvested).toFixed(2)}€ (${totalInvested > 0 ? ((currentValue / totalInvested - 1) * 100).toFixed(2) : 0}%)`,
    );
    ...
    if (totalInvested > currentValue * 2) {
      console.log(
        "⚠️  El capital invertido es más del doble del valor actual. Posibles causas:",
      );
      console.log(
        '   1. Se vendió parte de la inversión pero no se registró como "sell" o "withdraw"',
      );
      console.log(
        "   2. El operationAmount de alguna operación está incorrecto",
      );
    ...
    if (telefonica.quantity && telefonica.currentPrice) {
      ...
      if (Math.abs(expectedValue - currentValue) > 1) {
        console.log(
          `⚠️  Discrepancia: Valor calculado (${expectedValue.toFixed(2)}€) vs Valor actual (${currentValue.toFixed(2)}€)`,
        );
      }
    }
