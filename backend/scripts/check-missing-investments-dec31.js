...
async function checkMissingInvestmentsDec31() {
  try {
    ...
    for (const inv of investments) {
      ...
      console.log(`\n${inv.name} (${inv._id}):`);
      console.log(`  - Fecha compra: ${purchaseDate.toISOString().split('T')[0]}`);
      console.log(`  - Existía antes del 1/1/2026: ${existedBeforeYear ? 'SÍ' : 'NO'}`);
      ...
      if (existedBeforeYear) {
        ...
        if (variationDec31) {
          console.log(`  ✅ Tiene DailyVariation al 31/12: ${variationDec31.totalValue.toFixed(2)}€`);
        } else {
          ...
          if (lastVariation) {
            console.log(`  ⚠️  No tiene al 31/12, pero tiene más reciente (${lastVariation.date.toISOString().split('T')[0]}): ${lastVariation.totalValue.toFixed(2)}€`);
          } else {
            ...
            if (lastHistory) {
              console.log(`  ⚠️  No tiene DailyVariation, pero tiene InvestmentHistory (${lastHistory.date.toISOString().split('T')[0]}): ${lastHistory.totalValue.toFixed(2)}€`);
            } else {
              ...
              if (capital > 0) {
                console.log(`  ⚠️  No tiene registros de valor, capital invertido hasta 31/12: ${capital.toFixed(2)}€`);
              } else {
        ...
        console.log(`  ℹ️  Inversión nueva en 2026, no debería estar en el cálculo del inicio del año`);
      }
    }
