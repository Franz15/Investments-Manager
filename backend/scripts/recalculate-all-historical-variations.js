...
async function recalculateAll() {
  try {
    ...
    console.log(`📊 Encontradas ${investments.length} inversiones para recalcular\n`);
    ...
    for (const investment of investments) {
      try {
        console.log(`🔄 Procesando: ${investment.name} (${investment.symbol})...`);
        ...
        if (result.calculated > 0) {
          console.log(`   ✅ ${result.calculated} variaciones calculadas - ${result.message}`);
          success++;
        } else {
