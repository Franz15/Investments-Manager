...
async function debugJavierCalculations() {
  try {
    ...
    if (variationsDec31.length > 0) {
      console.log(
        `   Usando DailyVariation del 31/12: ${variationsDec31.length} registros`,
      );
      variationsDec31.forEach((v) => {
        yearStartValue += v.totalValue || 0;
        const inv = investments.find(
          (inv) => inv._id.toString() === v.investment.toString(),
        );
        console.log(
          `     - ${inv?.name || "Desconocida"}: ${v.totalValue.toFixed(2)}€`,
        );
      });
    } else {
      ...
      if (lastVariationBeforeYear) {
        ...
        console.log(
          `   Usando DailyVariation más reciente (${lastDate.toISOString().split("T")[0]}): ${variationsAtLastDate.length} registros`,
        );
        ...
        console.log(
          `   Usando InvestmentHistory: ${latestByInvestment.size} inversiones`,
        );
        latestByInvestment.forEach((totalValue, invId) => {
          ...
          console.log(
            `     - ${inv?.name || "Desconocida"}: ${totalValue.toFixed(2)}€`,
          );
        });
      }
    }
    ...
    const annualReturn =
      currentValue -
      yearStartValue -
      capitalAddedThisYear +
      capitalWithdrawnThisYear;
    const baseValue =
      yearStartValue + capitalAddedThisYear - capitalWithdrawnThisYear;
    ...
    console.log(`   Capital retirado: ${capitalWithdrawnThisYear.toFixed(2)}€`);
    console.log(
      `   Base (inicio + aportado - retirado): ${baseValue.toFixed(2)}€`,
    );
    console.log(
      `   RENDIMIENTO: ${annualReturn.toFixed(2)}€ (${annualReturnPercent.toFixed(2)}%)`,
    );
    ...
    if (historyFromDeleted.length > 0) {
      ...
      console.log(
        `   Estos registros NO deberían estar afectando los cálculos si el código filtra correctamente.`,
      );
    }
