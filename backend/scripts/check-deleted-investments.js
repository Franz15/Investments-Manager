...
async function checkDeletedInvestments() {
  try {
    ...
    allHistoryEntries.forEach(entry => {
      ...
      const isActive = activeInvestmentIds.some(id => id.toString() === invId);
    ...
    console.log(`   - De inversiones ACTIVAS: ${historyFromActiveInvestments.length}`);
    console.log(`   - De inversiones ELIMINADAS: ${historyFromDeletedInvestments.length}`);
    ...
    if (historyFromDeletedInvestments.length > 0) {
      ...
      Object.keys(deletedByInvestment).forEach(invId => {
        ...
        const totalOperations = entries.filter(e => ['creation', 'add', 'withdraw'].includes(e.operation)).length;
        ...
        console.log(`     * Fechas: ${new Date(entries[0].date).toISOString().split('T')[0]} - ${new Date(entries[entries.length - 1].date).toISOString().split('T')[0]}`);
      });
    }
    ...
    console.log(`   - De inversiones ACTIVAS: ${variationsFromActiveInvestments.length}`);
    console.log(`   - De inversiones ELIMINADAS: ${variationsFromDeletedInvestments.length}`);
    ...
    console.log(`  - InvestmentHistory de inversiones eliminadas: ${historyFromDeletedInvestments.length}`);
    console.log(`  - DailyVariation de inversiones eliminadas: ${variationsFromDeletedInvestments.length}`);
    
    if (historyFromDeletedInvestments.length > 0 || variationsFromDeletedInvestments.length > 0) {
