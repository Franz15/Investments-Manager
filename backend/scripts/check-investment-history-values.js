  ...
  try {
    ...
    for (const inv of investments) {
      ...
      if (purchaseDate < yearStart) {
        ...
        console.log(
          `   Valor actual: ${(inv.isAutomatedPortfolio ? inv.currentPrice : inv.quantity * inv.currentPrice).toFixed(2)}€`,
        );
        ...
        if (historyDec31.length > 0) {
          ...
          historyDec31.forEach((h) => {
            console.log(
              `     - ${h.date.toISOString().split("T")[0]} | ${h.operation} | totalValue: ${h.totalValue ? h.totalValue.toFixed(2) : "N/A"}€ | operationAmount: ${h.operationAmount ? h.operationAmount.toFixed(2) : "N/A"}€`,
            );
          });
        } else {
          ...
          if (lastHistory) {
            console.log(
              `   Último registro antes del 1/1: ${lastHistory.date.toISOString().split("T")[0]}`,
            );
            console.log(`     - Operación: ${lastHistory.operation}`);
            console.log(
              `     - totalValue: ${lastHistory.totalValue ? lastHistory.totalValue.toFixed(2) : "N/A"}€`,
            );
            console.log(
              `     - operationAmount: ${lastHistory.operationAmount ? lastHistory.operationAmount.toFixed(2) : "N/A"}€`,
            );
          } else {
