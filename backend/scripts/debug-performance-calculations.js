...
async function debugPerformanceCalculations() {
  try {
    ...
    capitalBreakdown.forEach((op) => {
      console.log(
        `  ${op.date} - ${op.operation} - ${op.investment}: ${op.amount >= 0 ? "+" : ""}${op.amount.toFixed(2)}€`,
      );
    });
    console.log("");
    ...
    console.log(
      `Rendimiento acumulado: ${(currentValue - totalInvestedCapital).toFixed(2)}€ (${totalInvestedCapital > 0 ? ((currentValue / totalInvestedCapital - 1) * 100).toFixed(2) : 0}%)\n`,
    );
    ...
    if (variationsDec31.length > 0) {
      ...
      variationsDec31.forEach((v) => {
        ...
        console.log(
          `  ${inv?.name || "N/A"}: ${value.toFixed(2)}€ (DailyVariation 31/12)`,
        );
      });
    } else {
      console.log(
      ...
      if (lastVariationBeforeYear) {
        ...
        console.log(
          `Encontradas ${variationsAtLastDate.length} variaciones del ${lastDate.toISOString().split("T")[0]}:`,
        );
    ...
    console.log(
      `Inversiones nuevas (desde el 1/1/2026): ${investments.length - oldInvestmentIds.size}`,
    );
    ...
    for (const inv of investments) {
      ...
      if (
        ...
        if (lastVariation && lastVariation.totalValue) {
          ...
          console.log(
            `  ${inv.name}: +${lastVariation.totalValue.toFixed(2)}€ (DailyVariation ${lastVariation.date.toISOString().split("T")[0]})`,
          );
        } else {
          ...
          if (lastHistory && lastHistory.totalValue) {
            ...
            console.log(
              `  ${inv.name}: +${lastHistory.totalValue.toFixed(2)}€ (InvestmentHistory ${lastHistory.date.toISOString().split("T")[0]})`,
            );
          }
        }
      }
    ...
    console.log(
      `Valor añadido de inversiones sin registro: ${missingInvestments.reduce((sum, inv) => sum + inv.value, 0).toFixed(2)}€`,
    );
    console.log(
      `Valor real al inicio del año (actualYearStartValue): ${actualYearStartValue.toFixed(2)}€`,
    );
    console.log("");
    ...
    console.log(
      `Aportaciones totales en 2026: ${capitalAddedThisYear.toFixed(2)}€`,
    );
    console.log(
      `  - A inversiones nuevas (creation): ${capitalAddedToNewInvestments.toFixed(2)}€`,
    );
    console.log(
      `  - A inversiones existentes (add): ${capitalAddedToOldInvestments.toFixed(2)}€`,
    );
    ...
    console.log(
      `Valor actual de inversiones antiguas: ${currentValueOfOldInvestments.toFixed(2)}€`,
    );
    console.log(
      `Valor actual de inversiones nuevas: ${currentValueOfNewInvestments.toFixed(2)}€`,
    );
    ...
    console.log(
      `  annualReturn = currentValue - actualYearStartValue - capitalAddedToOldInvestments + capitalWithdrawnThisYear`,
    );
    console.log(
      `  annualReturn = ${currentValue.toFixed(2)} - ${actualYearStartValue.toFixed(2)} - ${capitalAddedToOldInvestments.toFixed(2)} + ${capitalWithdrawnThisYear.toFixed(2)}`,
    );
    ...
    console.log(
      `Rendimiento acumulado: ${(currentValue - totalInvestedCapital).toFixed(2)}€`,
    );
    console.log("");
    console.log(
      `Valor al inicio del año (actualYearStartValue): ${actualYearStartValue.toFixed(2)}€`,
    );
    console.log(
      `Aportaciones a inversiones existentes en 2026: ${capitalAddedToOldInvestments.toFixed(2)}€`,
    );
    console.log(`Retiros en 2026: ${capitalWithdrawnThisYear.toFixed(2)}€`);
    console.log(
      `Rendimiento anual: ${annualReturn.toFixed(2)}€ (${annualReturnPercent.toFixed(2)}%)`,
    );
    console.log("");
