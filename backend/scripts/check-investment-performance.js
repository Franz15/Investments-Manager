    ...
    for (const userId of users) {
      ...
      for (const inv of investments) {
        const invHistory = allHistoryEntries.filter(
          entry => entry.investment?._id?.toString() === inv._id.toString() ||
        ...
        const returnPercent = investedCapital > 0 ? ((currentValue / investedCapital) - 1) * 100 : 0;
      ...
      console.log('-'.repeat(80));
      console.log('Inversión'.padEnd(40) + 'Cuenta'.padEnd(20) + 'Subcuenta'.padEnd(20) + 'Capital'.padEnd(15) + 'Actual'.padEnd(15) + 'Rendimiento');
      console.log('-'.repeat(80));
      ...
      for (const perf of investmentPerformance) {
        ...
        const returnStr = perf.returnAmount >= 0
      ...
      const totalReturnPercent = totalInvested > 0 ? ((totalCurrent / totalInvested) - 1) * 100 : 0;
      console.log(
        'TOTAL'.padEnd(40) +
        ''.padEnd(20) +
        ''.padEnd(20) +
      ...
      for (const [key, data] of Object.entries(bySubAccount)) {
        ...
        const subReturnPercent = data.totalInvested > 0 ? ((data.totalCurrent / data.totalInvested) - 1) * 100 : 0;
        console.log(`\n${key} (${data.account}):`);
        console.log(`  Capital invertido: ${data.totalInvested.toFixed(2)}€`);
        console.log(`  Valor actual: ${data.totalCurrent.toFixed(2)}€`);
        console.log(`  Rendimiento: ${subReturn >= 0 ? '+' : ''}${subReturn.toFixed(2)}€ (${subReturnPercent.toFixed(2)}%)`);
        if (subReturn < -1000) {
          console.log(`  ⚠️  PÉRDIDA SIGNIFICATIVA`);
        }
        for (const inv of data.investments) {
          console.log(`    - ${inv.name}: ${inv.returnAmount >= 0 ? '+' : ''}${inv.returnAmount.toFixed(2)}€`);
        }
      }
