...
async function createTestDCAInvestment() {
  try {
    ...
    let testAccount = await Account.findOne({ user: userId, name: 'Cuenta Prueba DCA' });
    ...
    if (!testInvestment) {
      ...
      console.log('   - Próxima compra:', testInvestment.dcaNextDate.toLocaleDateString('es-ES'));
    } else {
    ...
    console.log('2. O esperar hasta mañana para que se ejecute automáticamente');
