    ...
    for (const userId of users) {
      ...
      for (const entry of allHistoryEntries) {
        ...
        if (entry.operation === "creation" || entry.operation === "add") {
          ...
          if (amount < 0) {
            console.log(
              `⚠️  OPERACIÓN ${entry.operation.toUpperCase()} CON AMOUNT NEGATIVO:`,
            );
            console.log(
        ...
        if (ops.length > 0) {
          ...
          if (opType === "creation" || opType === "add") {
            ...
            for (const op of sorted) {
              console.log(
                `      - ${op.date.toISOString().split("T")[0]}: ${op.amount.toFixed(2)}€ - ${op.investment} ${op.calculatedFrom || ""}`,
              );
            }
      ...
      console.log("-".repeat(80));
      console.log(
        `  Capital añadido (creation + add): ${totalAdded.toFixed(2)}€`,
      );
      console.log(
        `  Capital restado (withdraw + sell): ${totalSubtracted.toFixed(2)}€`,
      );
      console.log(
        `  Capital neto invertido: ${(totalAdded - totalSubtracted).toFixed(2)}€`,
      );
      ...
      const totalReturnPercent =
        netInvestedCapital > 0
          ? (currentValue / netInvestedCapital - 1) * 100
          : 0;

      console.log(`  Valor actual total: ${currentValue.toFixed(2)}€`);
      console.log(
        `  Rendimiento acumulado: ${totalReturn.toFixed(2)}€ (${totalReturnPercent.toFixed(2)}%)`,
      );
      console.log("");
        ...
        if (entry.operation === "creation" || entry.operation === "add") {
          ...
          if (entry.totalValue && Math.abs(amount - entry.totalValue) > 100) {
            ...
            if (investment) {
              ...
              console.log(
                `     operationAmount: ${entry.operationAmount || "null/undefined"}`,
              );
              console.log(`     Amount calculado: ${amount.toFixed(2)}€`);
              console.log(`     totalValue: ${entry.totalValue.toFixed(2)}€`);
              console.log(
                `     Diferencia: ${Math.abs(amount - entry.totalValue).toFixed(2)}€`,
              );
            }
          }
        }
