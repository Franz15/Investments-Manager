  ...
  try {
    ...
    for (const inv of investments) {
      ...
      if (lastVariation && lastVariation.totalValue) {
        ...
        console.log(
          `   - ${inv.name}: ${lastVariation.totalValue.toFixed(2)}€ (DailyVariation)`,
        );
      } else {
        ...
        if (lastHistory && lastHistory.totalValue) {
          ...
          console.log(
            `   - ${inv.name}: ${lastHistory.totalValue.toFixed(2)}€ (InvestmentHistory)`,
          );
        } else {
          ...
          console.log(
            `   - ${inv.name}: ${Math.max(0, capital).toFixed(2)}€ (capital acumulado)`,
          );
        }
      }
    }
    ...
    console.log(
      `2. Valor de mercado actual (modelo Investment): ${currentMarketValue.toFixed(2)}€`,
    );
    console.log(
      `3. Valor usando DailyVariation (último día): ${valueFromDailyVariation.toFixed(2)}€`,
    );
    console.log(`\nDIFERENCIAS:`);
    console.log(
      `   - Capital invertido vs Valor de mercado: ${(currentMarketValue - initialValue).toFixed(2)}€ (ganancia/pérdida)`,
    );
    console.log(
      `   - Capital invertido vs DailyVariation: ${(valueFromDailyVariation - initialValue).toFixed(2)}€`,
    );
    console.log(
      `   - Valor de mercado vs DailyVariation: ${(currentMarketValue - valueFromDailyVariation).toFixed(2)}€`,
    );
