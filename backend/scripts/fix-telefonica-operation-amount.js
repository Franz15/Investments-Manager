...
async function fixTelefonicaOperationAmount() {
  try {
    ...
    console.log(
      `   Valor actual: ${(telefonica.quantity || 0) * (telefonica.currentPrice || 0)}€`,
    );
    console.log("");
    ...
    console.log(
      `   Precio: ${creationEntry.currentPrice || creationEntry.operationPrice || "N/A"}`,
    );
    ...
    const correctPrice =
      telefonica.purchasePrice ||
      telefonica.averagePurchasePrice ||
      telefonica.currentPrice ||
      0;
    ...
    console.log(
      `   Operation Amount correcto: ${correctOperationAmount.toFixed(2)}€`,
    );
    console.log("");
    ...
    if (
      creationEntry.operationPrice === null ||
      creationEntry.operationPrice === undefined
    ) {
      creationEntry.operationPrice = correctPrice;
    }
    if (
      creationEntry.currentPrice === null ||
      creationEntry.currentPrice === undefined
    ) {
    ...
    console.log(
      `   Operation Amount: ${updatedEntry.operationAmount.toFixed(2)}€`,
    );
    ...
    const newReturnPercent =
      correctOperationAmount > 0
        ? (currentValue / correctOperationAmount - 1) * 100
        : 0;
    ...
    console.log(
      `   Rendimiento: ${newReturn >= 0 ? "+" : ""}${newReturn.toFixed(2)}€ (${newReturnPercent.toFixed(2)}%)`,
    );
    console.log("");
