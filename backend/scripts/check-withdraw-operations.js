  ...
  try {
    ...
    for (const userId of users) {
      ...
      for (const op of withdrawOperations) {
        const invId = op.investment?._id?.toString() || op.investment?.toString() || 'N/A';
        ...
        const investment = await Investment.findOne({ _id: invId, user: userId });
        let currentValue = 0;
        if (investment) {
        ...
        console.log(`  📅 ${op.date.toISOString().split('T')[0]} - ${op.operation.toUpperCase()}`);
        console.log(`     Inversión: ${invName}`);
        console.log(`     Amount calculado: ${amount.toFixed(2)}€`);
        console.log(`     operationAmount (BD): ${op.operationAmount || 'null/undefined'}`);
        ...
        if (isSuspicious) {
          console.log(`     ⚠️  SOSPECHOSA: El amount (${amount.toFixed(2)}€) es > 150% del valor actual (${currentValue.toFixed(2)}€)`);
        }
        console.log('');
      }
      ...
      for (const [invId, data] of Object.entries(operationsByInvestment)) {
        ...
        for (const op of data.operations) {
          console.log(`      - ${op.date.toISOString().split('T')[0]}: ${op.amount.toFixed(2)}€ (${op.operation})`);
        }
      }

      console.log('\n💰 Total general restado:', totalSubtracted.toFixed(2), '€');
      console.log('');
      ...
      console.log(`  Rendimiento acumulado: ${totalReturn.toFixed(2)}€ (${((totalReturn / totalInvestedCapital) * 100).toFixed(2)}%)`);
      console.log(`  Total restado por withdraw/sell: ${totalSubtracted.toFixed(2)}€`);
      console.log('');
    }
