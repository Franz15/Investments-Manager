...
async function createDCAMockData() {
  try {
    ...
    if (!investment) {
      console.error('❌ No se encontró la inversión de prueba. Ejecuta primero create-test-dca-investment.js');
    ...
    console.log(`   - Valor total: ${investment.quantity * investment.currentPrice}€\n`);
    ...
    let quantity = 10;
    let averagePurchasePrice = 150.00;
    let totalInvested = 1500.00; // 10 acciones * 150€
    ...
    console.log(`✅ Registro inicial creado: ${startDate.toLocaleDateString('es-ES')} - ${quantity} acciones a ${currentPrice}€`);
    ...
    while (dcaDate <= today) {
      ...
      const priceVariation = (Math.random() * 0.10 - 0.05); // -5% a +5%
      ...
      const newAveragePrice = ((quantity * averagePurchasePrice) + (quantityToBuy * currentPrice)) / newQuantity;
      ...
      console.log(`✅ Compra DCA simulada: ${dcaDate.toLocaleDateString('es-ES')}`);
      console.log(`   - Precio: ${currentPrice.toFixed(2)}€`);
      console.log(`   - Cantidad comprada: ${quantityToBuy.toFixed(4)} unidades`);
      ...
      console.log(`   - Ganancia/Pérdida: ${((quantity * currentPrice) - totalInvested).toFixed(2)}€ (${(((quantity * currentPrice) - totalInvested) / totalInvested * 100).toFixed(2)}%)\n`);
    ...
    if (historyEntries.length > 0) {
      ...
      console.log(`✅ ${historyEntries.length} compras DCA guardadas en el historial\n`);
    }
    ...
    console.log(`   - Cantidad total: ${quantity.toFixed(4)} unidades`);
    console.log(`   - Precio medio de compra: ${averagePurchasePrice.toFixed(2)}€`);
    console.log(`   - Precio actual: ${currentPrice.toFixed(2)}€`);
    console.log(`   - Valor total actual: ${(quantity * currentPrice).toFixed(2)}€`);
    console.log(`   - Capital invertido total: ${totalInvested.toFixed(2)}€`);
    console.log(`   - Ganancia/Pérdida: ${((quantity * currentPrice) - totalInvested).toFixed(2)}€`);
    console.log(`   - Rendimiento: ${(((quantity * currentPrice) - totalInvested) / totalInvested * 100).toFixed(2)}%`);
    console.log(`   - Próxima compra DCA: ${dcaDate.toLocaleDateString('es-ES')}\n`);
