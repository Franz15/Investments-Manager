...
async function cleanupDuplicateHistoryUpdates() {
  ...
  try {
    ...
    if (investments.length === 0) {
      console.log(
        `ℹ️ No se han encontrado inversiones para el usuario "${userId}".`,
      );
    ...
    console.log(
      `📈 Entradas de historial de tipo "update" encontradas: ${updates.length}\n`,
    );
    ...
    for (const [key, entries] of groups.entries()) {
      ...
      console.log(
        `🧹 Inversión ${invId} - Día ${day}: ${entries.length} entradas -> conservar 1, eliminar ${toDelete.length}`,
      );

      const deleteIds = toDelete.map((e) => e._id);
      const res = await InvestmentHistory.deleteMany({
        _id: { $in: deleteIds },
      });
    ...
    console.log("\nResumen:");
    console.log(
      `🗂 Grupos (inversión + día) con duplicados: ${totalGroupsWithDuplicates}`,
    );
