...
router.get('/', requireAdmin, async (req, res) => {
  try {
    const users = await User.find({}).select('id name role permissions lastLogin').sort({ id: 1 });
    res.json(users);
  } catch (error) {
  ...
  try {
    ...
    if (user.id === 'javier' && req.userId === 'javier') {
      return res.status(400).json({
        message: 'No puedes modificar tus propios permisos de administrador desde este panel.',
      });
    }
    ...
    user.permissions = {
      ...
      canAccessFinances:
        typeof permissions.canAccessFinances === 'boolean'
          ? permissions.canAccessFinances
          : (user.permissions?.canAccessFinances ?? true),
      canAccessBusinesses:
        typeof permissions.canAccessBusinesses === 'boolean'
          ? permissions.canAccessBusinesses
          : (user.permissions?.canAccessBusinesses ?? true),
      canChangePassword:
...
router.patch('/me/language', async (req, res) => {
  try {
    ...
    const user = await User.findOneAndUpdate({ id: req.userId }, { language }, { new: true });
...
router.patch('/me/color', async (req, res) => {
  try {
    ...
    if (!color || !/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(color)) {
      return res.status(400).json({
        message: 'Color inválido. Debe ser un código hexadecimal válido (ej: #3b82f6)',
      });
    }

    const user = await User.findOneAndUpdate({ id: req.userId }, { color }, { new: true });
...
router.patch('/me/password', async (req, res) => {
  try {
    ...
    const user = await User.findOne({ id: req.userId }).select('+password permissions');
...
// DELETE eliminar un usuario (solo admin — no se puede eliminar a Javier ni a uno mismo)
router.delete('/:userId', requireAdmin, async (req, res) => {
  try {
    const { userId } = req.params;

    if (userId === 'javier') {
      return res.status(400).json({ message: 'No se puede eliminar al administrador principal.' });
    }
...
router.post('/:userId/reset-password', requireAdmin, async (req, res) => {
  try {
    ...
    res.json({
      ...
      generatedPassword:
        typeof newPassword === 'string' && newPassword.length >= 4 ? undefined : finalPassword,
    });
  } catch (error) {
    console.error('Error al resetear contraseña de usuario:', error);
    res.status(500).json({ message: 'Error al resetear la contraseña del usuario' });
  }
});
