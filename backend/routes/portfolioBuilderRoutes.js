...
function getExcludedIsinsForCategory(excludedFundIsinsByCategory, category) {
  const raw =
    excludedFundIsinsByCategory && excludedFundIsinsByCategory[category];
  if (Array.isArray(raw)) return raw;
  if (raw && typeof raw === "object" && raw.constructor?.name === "Map") {
    return Array.from(raw.values()).flat();
  }
  ...
  try {
    ...
    res.json({
      ...
      funds: funds.map((f) => {
        ...
        const excludedIsins = getExcludedIsinsForCategory(
          excludedByCat,
          f.category,
        );
...
router.put("/config", async (req, res) => {
  try {
    ...
    if (
      extraFundIsinsByCategory != null &&
      typeof extraFundIsinsByCategory === "object"
    ) {
      config.extraFundIsinsByCategory = extraFundIsinsByCategory;
    }
    if (
      excludedFundIsinsByCategory != null &&
      typeof excludedFundIsinsByCategory === "object"
    ) {
