...
router.get('/', async (req, res) => {
  try {
    ...
    if (subAccountId) {
      ...
      const subAccounts = await SubAccount.find({ account: accountId, user: req.userId });
...
router.post('/', async (req, res) => {
  try {
    ...
    const subAccount = await SubAccount.findOne({ _id: req.body.subAccount, user: req.userId });
    if (subAccount) {
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const subAccount = await SubAccount.findOne({ _id: transaction.subAccount, user: req.userId });
    if (subAccount) {
